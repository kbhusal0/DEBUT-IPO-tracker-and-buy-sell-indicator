...
def fetch_and_store_ticker(conn, ticker: str, ipo_date_str: str):
    ...
    # Drop any rows where Close is missing (e.g. halted trading days,
    # very recently listed tickers, or a mistyped/invalid ticker that
    # partially resolves). Inserting a NULL close_price would violate
    # the database's NOT NULL constraint.
    hist = hist[hist["Close"].notna()]

    if hist.empty:
        print(f"  [WARN] {ticker} returned only missing/invalid price data "
              f"(double-check the ticker symbol is correct)")
        return None, None
