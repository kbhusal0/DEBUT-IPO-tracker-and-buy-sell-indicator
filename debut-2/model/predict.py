...
def predict_signal(offer_price: float, day1_close: float | None):
    ...
    # pandas represents missing values as NaN (a float), not Python's
    # None — pd.isna() catches both cases so we don't crash on NaN input.
    if model is None or day1_close is None or pd.isna(day1_close):
        return None
